...
class Kinetic {
    ...
    get(key, range, callback) {
        ...
        // range based on MD is not accurate for our MP object, is this imp
        // since it was passed to this function?
            ...
            socket.sock.write(header, err => {
                ...
                return this._events.once('getResponse', (err, chunk) => {
                    ...
                    return callback(err, new stream.Readable({
                        read() {
                            if (isMaster) {
                                // aggregate chunks to push to stream
                                const keysArray = _parseChunk(chunk);
                                keysArray.forEach((key) => {
                                    console.log(key);
                                });
                            }
                            else {
                                // create stream for normal object
                                this.push(chunk);
                                this.push(null);
                            }
                        },
    ...
    // _getViableHeader(host, port, socket) {
    //     this._sockets.forEach(socket => {
    //         if (socket.drive.host === host && socket.drive.port) {
